    ...
    public function generateTableStructure($table)
    {
        ...
        if (!empty($indexs)) {
            foreach ($indexs as $index => $item) {
                $str = '$this->createIndex(\'' . $index . '\',\'{{%' . $this->getTableName($table) . '}}\',\'' . implode(', ',
                        $item['columns']) . '\',' . $item['unique'] . ');';
            ...
            foreach ($tableSchema->foreignKeys as $fk) {
                ...
                foreach ($fk as $k => $v) {
                    if ($k == '0') {
                        ...
                        $str .= '\'fk_' . $this->getTableName($link_table) . '_' . explode('.',
                                microtime('usec'))[1] . '_' . substr("000" . sizeof($foreignKeys), 2) . "',";
